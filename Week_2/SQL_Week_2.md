...
```sql
SELECT CONCAT(ST.firstName,' ',ST.lastName) AS 'Student name', G.CLASS_NAME, G.GRADE
FROM students ST
    JOIN grades G
...
SELECT *
FROM categories 
WHERE CategoryName NOT IN ('Beverages', 'Dairy Products');
```
